...
print(f"the shape of Vmod is {h1e_act.shape},there are {nact_2e} active alpha occupied orbitals and {nact_2e_B} beta occupied orbitals")
h1e_act -= 1/2*np.einsum('acbb->ac', h2e_aa[:nact, :nact, :nact_2e, :nact_2e])
h1e_act += 1/2*np.einsum('abbc->ac', h2e_aa[:nact, :nact_2e, :nact_2e, :nact])
h1e_act -= np.einsum('acbb->ac', h2e_ba[:nact, :nact, :nact_2e_B, :nact_2e_B])
h1e_act += np.einsum('abbc->ac', h2e_ba[:nact, :nact_2e_B, :nact_2e_B, :nact])
h1e_act -= 1/2*np.einsum('acbb->ac', h2e_bb[:nact, :nact, :nact_2e_B, :nact_2e_B])
h1e_act += 1/2*np.einsum('abbc->ac', h2e_bb[:nact, :nact_2e_B, :nact_2e_B, :nact])
...
enuc -=  1/2*np.trace(h1e_act[:nact_2e-2, :nact_2e-2])
print(f"The nuclear repulsion is {enuc}")
